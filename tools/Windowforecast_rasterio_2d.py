...
if __name__ == '__main__':
    ...
    block_w = 512
    block_h = 512
    area_perc = 0.8  # 预测区域，或者说非重叠区域

    # work_dir = "/data/study_Irrigation/segmentation_models.pytorch/Code_my"
    # inform_dir = "/data/study_Irrigation/Wa_256"
    # mean_file = os.path.join(inform_dir, 'imgset_mean_2d.txt')
    # std_file = os.path.join(inform_dir, 'imgset_std_2d.txt')
    #
    ...
    #TifPath_8 = "/data/study_Irrigation/Wa_img_all_84/Wa_val_8_022.tif"
    # TifPath_9 = "/data/study_Irrigation/Wa_img_all_84/Wa_val_9_022.tif"
    # TifPath_10 = "/data/study_Irrigation/Wa_img_all_84/Wa_val_10_022.tif"

    "模型权重所在路径"
    # 可以是多个，存储在list中
    model_paths = [r"D:\MasterProgram\paper2\MyNET\log\Compare\T\ex8_20250124-035606\best_f1\188_net_g.pth",
                   r"D:\MasterProgram\paper2\MyNET\log\Compare\T\ex8_20250124-033317\best_f1\150_net_g.pth"]
    ...
    trainer = base_trainer(train_flag=False)
    model = trainer.net_g
    model.eval()
    "指定cuda"
    ...
    for i in range(len(TifArray_7)):
        ...
        for j in range(len(TifArray_7[0])):
            ...
            image_concat = torch.from_numpy(image_concat).to(DEVICE).unsqueeze(0)
            pred = np.zeros((1, block_w, block_h))
            for model_path in model_paths:
                # 策略2：使用一个模型只做一次预测,只对原图做预测
                model.load_state_dict(torch.load(model_path))
                with torch.no_grad():
                    pred_temp = model(image_concat).cpu().numpy().squeeze(0)
                    pred = pred + pred_temp
